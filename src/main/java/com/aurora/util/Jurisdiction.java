...
public class Jurisdiction {
	...
	public static boolean hasJurisdiction(String menuUrl){
		...
		List<Menu> menuList = (List<Menu>)session.getAttribute("roleMenuList"); //获取菜单列表
		return readMenu(menuList,menuUrl);
	}
	...
	public static boolean readMenu(List<Menu> menuList,String menuUrl){
		for(int i=0;i<menuList.size();i++){
			if (menuList.get(i).getMenuURL() != null && menuList.get(i).getMenuURL().split(".do")[0].equals(menuUrl.split(".do")[0])) {
				...
				if(readMenu(menuList.get(i).getSubMenu(),menuUrl)){//继续排查其子菜单
					return true;
				}
			}
		}
	 ...
	 * 按钮权限(方法中校验)
	 * @param menuUrl  菜单路径
	 * @param type  类型(add、del、edit、see)
	 * @return
	 */
	@SuppressWarnings("unchecked")
	public static boolean buttonJurisdiction(String menuUrl, String type){
		...
		Session session = getSession();
		List<Menu> menuList = (List<Menu>)session.getAttribute(userName + Const.SESSION_ALL_MENU_LIST); //获取菜单列表
		...
		for(int i=0;i<menuList.size();i++){
			if(menuList.get(i).getMenuURL().split(".do")[0].equals(menuUrl.split(".do")[0])){ //访问地址与菜单地址循环匹配，如何匹配到就进一步验证，如果没匹配到就不处理(可能是接口链接或其它链接)
				if(!menuList.get(i).isHasMenu()){				//判断有无此菜单权限
					...
					Map<String, String> map = (Map<String, String>)session.getAttribute(userName + Const.SESSION_RIGHTS);//按钮权限(增删改查)
					...
					if("add".equals(type)){
						...
						return ((RightsHelper.testRights(map.get("sees"), menuID)) || isAdmin);
					}
				}
			}else{
	...
	public static String getUserName(){
		return getSession().getAttribute(Const.SESSION_USER_NAME).toString();
	}
	...
	public static String getUserEmail(){
		return getSession().getAttribute(Const.SESSION_USER_EMAIL).toString();
	}
	...
	public static Map<String, String> getHC(){
		return (Map<String, String>)getSession().getAttribute(getUserName() + Const.SESSION_RIGHTS);
	}
