...
<html lang="en">
	...
	<body id="goodsEntry-body">
		...
		<h1>品牌查看修改</h1>
		<div id="entry-box">
			<i id="brandId" data-brand="${pd.brandID}"></i>
			<h2 class="title">1.品牌信息</h2>
			<div class="info-box">
				<h3><i>*</i>品牌类目：</h3>
				<div class="info">
					<select id="brand-cate" style="width:100px;height:30px">
						<c:forEach items="${brandCategory}" var="brand">
							<option <c:if test="${brandByID[0].brand_category_id == brand.brand_category_id}">selected="selected"</c:if> cateID="${brand.brand_category_id}">${brand.brand_category_name}</option>
						</c:forEach>			
					</select>
				</div>
			</div>
			<div class="info-box" style="float:left;width:500px;">
				...
				<div class="info" style="width:300px">
					<input id="brandName" value="${brandByID[0].brand_name}" style="width:200px"/>
				</div>
			</div>
				...
				<div class="info" style="width:200px">
					<select id="brandC" style="width:100px;height:30px">
						<c:forEach items="${brandCountry}" var="brand">
							<option <c:if test="${brand.country_cname == brandByID[0].brand_country_cname}">selected="selected"</c:if> brandCId="${brand.brandc_id}" nationalFlag="${brand.national_flag}" countryEName="${brand.country_ename}">${brand.country_cname}</option>
						</c:forEach>			
					</select>
				</div>
			</div>
			<div class="info-box">
				<h3><i>*</i>一句话介绍：</h3>
				<div class="info">
					<input type="text" class="inpt-short words-num" id="brandDescribe1" onblur="wordsNum(this)" value="${brandByID[0].brand_describe1}"/>
			...
			<div class="info-box">
				<h3><i>*</i>详情介绍：</h3>
				<div class="info">
					<textarea type="text" class="inpt-short words-num" id="brandDescribe2" onblur="wordsNum(this)" placeholder="详情介绍">${brandByID[0].brand_describe2}</textarea>
				...
				<div class="info upload">
					<div class="uploadImg" style="float:none">
						...
						<div class="preview" id="brandMap" onclick="click_img(this)">
							<img class="preview-img" src="${brandByID[0].brand_map}">
						</div>
					...
					<div class="uploadImg" style="float:none">
						...
						<div class="preview" id="recommendMap" onclick="click_img(this)">
							<img class="preview-img" src="${brandByID[0].recommend_map}">
						</div>
						<input id="" type="file" name="file" class="hidden" accept="image/jpg,image/jpeg,image/png,image/gif" onchange="previewImage(this)" />
					...
					<div class="uploadImg" id="brandAd">
						<span class="delete-img glyphicon glyphicon-remove delete-img-active" onclick="delete_img(this)"></span>
						<div class="preview" id="advertiseMap" onclick="click_img(this)">
							<img class="preview-img" src="${brandByID[0].advertise_map}">
						</div>
						<input id="" type="file" name="file" class="hidden" accept="image/jpg,image/jpeg,image/png,image/gif" onchange="previewImage(this)" />
...
//				});			
			...
			$(function(){
				...
				$('#save-good').on('click',function(){
					...
					brandVo.brandId = $('#brandId').data('brand');
...
//					return console.log(brandVo);
					$.ajax({
						type:'post',
						url:'brandManage/updateBrand',
						data:brandVo,
						dataType:'json',
						success:function(data){
							if(data.result == 'success'){
