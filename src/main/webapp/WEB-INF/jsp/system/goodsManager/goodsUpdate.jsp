...
<html lang="en">
	...
	<body id="goodsEntry-body">
		...
		<div id="entry-box" goodsID="${goods.goodsID}">
			<br/>
			<div class="category">
				<span><i>*</i>类目</span>
				<select id="category1" onchange="selectCate1(this)">
					...
					<c:forEach items="${goodsCategory1}" var="good">
						<option <c:if test="${goods.goodsCommon.category1ID == good.category_id}">selected="selected"</c:if> value="${good.category_id}">${good.category_name}</option>
					</c:forEach>					
				</select>
				<select id='category2' onchange="selectCate2(this)">
					<option value="${goods.goodsCommon.category2ID}">${goods.goodsCommon.category2}</option>
				</select>
				<select id='category3'>
					<option value="${goods.goodsCommon.category3ID}">${goods.goodsCommon.category3}</option>
				</select>
			...
			<div class="info-box">
				<h3><i>*</i>贸易方式：</h3>
				<div class="info">
					<input <c:if test="${goods.shipType == '1'}">checked="checked"</c:if> name="ship" type="radio" value="1" shipTypeN="保税仓直邮" class="inpt-radio" /> <i class="radio-right">保税仓直邮</i>
					<input <c:if test="${goods.shipType == '2'}">checked="checked"</c:if> name="ship" type="radio" value="2" shipTypeN="海外直邮" class="inpt-radio" /> <i class="radio-right">海外直邮</i>
					<input <c:if test="${goods.shipType == '3'}">checked="checked"</c:if> name="ship" type="radio" value="3" shipTypeN="国内现货" class="inpt-radio" /> <i class="radio-right">国内现货</i>
				</div>
			</div>
			<div class="info-box">
				<h3><i>*</i>商品主标题：</h3>
				<div class="info">
					<input type="text" class="inpt-long words-num" id="goodsName" onblur="wordsNum(this)" placeholder="商品名称" value="${goods.goodsDetails.goodsName}"/>
			...
			<div class="info-box">
				<h3><i>*</i>商品副标题：</h3>
				<div class="info">
					<input type="text" class="inpt-long words-num" id="gDescribe1" onblur="wordsNum(this)" placeholder="商品辅助说明" value="${goods.goodsDetails.describe}"/>
						...
						<h5>
							...
							<select id="brandName" style="width:300px;height:30px">
								<c:forEach items="${brand}" var="brand">
									<option value="${brand.brand_id}" <c:if test="${goods.goodsCommon.brandID == brand.brand_id}">selected="selected"</c:if>>${brand.brand_name}</option>
								</c:forEach>
							</select>
						</h5>
						<h5>
							<label><i>*</i>品牌国家</label>
							<input id="productArea" value="${goods.goodsDetails.productArea}"/>
						</h5>
						<h5>
							<label><i>*</i>重量(kg)</label>
							<input id="weight" type="number" value="${goods.goodsDetails.weight}" placeholder="只允许输入数字"/>
						</h5>
						<h5>
							<label><i>*</i>体积</label>
							<input id="volume" type="text" value="${goods.goodsDetails.volume}" placeholder="请自行填入单位"/>
						</h5>
						<h5>
							<label><i>*</i>条形码</label>
							<input id="goodsCode" value="${goods.goodsDetails.goodsCode}"/>
						</h5>
						<div id="property">
							<c:forEach items="${property}" var="good" varStatus="vs">
								<c:if test="${vs.index%2 == 0 }">
									<h5><label>${good}</label>
								</c:if>
								<c:if test="${vs.index%2 == 1 }">	
									<input class='property-list' value="${good}" onkeyup="this.value=this.value.replace(/\,/g,'')"/></h5>
								</c:if>
							</c:forEach>
						</div>
					</div>
			...
			<div class="info-box">
				<h3><i>*</i>包装规格：</h3>
				<div class="info">
					<input type="text" id="norm" value="${goods.norm}"/>
				</div>
			</div>
			<div class="info-box">
				<h3><i>*</i>商品关键词：</h3>
				<div class="info">
					<div id="good-keyword-box">
						<c:if test="${goods.goodsCommon.keyword1 != null}"><b class="good-keyword">${goods.goodsCommon.keyword1}<i class="delete-keyword glyphicon glyphicon-remove"></i></b></c:if>
						<c:if test="${goods.goodsCommon.keyword2 != null}"><b class="good-keyword">${goods.goodsCommon.keyword2}<i class="delete-keyword glyphicon glyphicon-remove"></i></b></c:if>
						<c:if test="${goods.goodsCommon.keyword3 != null}"><b class="good-keyword">${goods.goodsCommon.keyword3}<i class="delete-keyword glyphicon glyphicon-remove"></i></b></c:if>
						<c:if test="${goods.goodsCommon.keyword4 != null}"><b class="good-keyword">${goods.goodsCommon.keyword4}<i class="delete-keyword glyphicon glyphicon-remove"></i></b></c:if>
						<c:if test="${goods.goodsCommon.keyword5 != null}"><b class="good-keyword">${goods.goodsCommon.keyword5}<i class="delete-keyword glyphicon glyphicon-remove"></i></b></c:if>
					</div>
					<input type="text" id="good-keyword-inpt" onkeyup="value=value.replace(/\s/g,'')"/>
					<a href="javascript:;" id="add-good-keyword">添加新关键词</a>
				</div>
			</div>
			<div class="info-box">
				<h3><i>*</i>商品展示图：</h3>
				...
				<div class="info upload">
					<div class="uploadImg">
						<span class="delete-img glyphicon glyphicon-remove delete-img-active" onclick="delete_img(this)"></span>
						<div class="preview" onclick="click_img(this)">
							<img class='preview-img' src="${goods.goodsDetails.mainMap}"/>
						</div>
				...
				<div class="info upload" id="map6">
					...
					<c:forEach items="${map6}" var="img">
						<div class="uploadImg hasMap6">
				...
				<div class="info upload" id="advertiseMap">
					...
					<c:forEach items="${advertiseMap}" var="img">
						<div class="uploadImg hasMap6">
							<span class="delete-img glyphicon glyphicon-remove delete-img-active" onclick="delete_imgs(this)"></span>
							<div class="preview">
								<img class='preview-img' src="${img}"/>
				...
				<div class="info">
					<table class="table table-bordered">
						<tbody>
							...
							<tr>
								<td><input id="goodsPrice1" type="number" value="${goods.goodsPrice1}"/></td>
								<td><input id="rnum1" type="number" value="${goods.rnum1}"/></td>
								<td><input id="rnum2" type="number" value="${goods.rnum2}"/></td>
							</tr>
							<tr>
								<td><input id="goodsPrice2" type="number" value="${goods.goodsPrice2}"/></td>
								<td><input type="number" value="${goods.rnum2 + 1}"/></td>
								<td><input id="rnum3" type="number" value="${goods.rnum3}"/></td>
							</tr>
						</tbody>
					</table>
					<div class="info-box">
						...
						<div class="info" style="width:500px">
							<input value="1" name="discount" type="radio" class="inpt-radio" <c:if test="${goods.discount != '0'}">checked="checked"</c:if>/> <i class="radio-right">支持折扣</i>
							<input value="2" name="discount" type="radio" class="inpt-radio" <c:if test="${goods.discount == '0' || goods.discount == '100'}">checked="checked"</c:if>/> <i class="radio-right">不支持折扣</i>
						</div>
					</div>
					<div class="info-box" id="discount-radio" <c:if test="${goods.discount != '0' && goods.discount != '100'}">style="display: block;"</c:if>>
						<h3><i>*</i>折扣比例：</h3>
						<div class="info" style="width:500px">
							<input type="number" class="inpt-short" id="discount" value="${goods.discount}"/><i style="color:#e10000">（只限100内整数,无需输入百分号。）</i>
						</div>
					</div>
					<div class="exw-box">阶段二价格是否是exw报价<input type="radio" name="exw" value="1" <c:if test="${goods.exw == '1'}">checked="checked"</c:if>/>是<input type="radio" name="exw" value="2" <c:if test="${goods.exw == '2'}">checked="checked"</c:if>/>不是</div>
				</div>
			</div>
			<div class="info-box">
				<h3><i>*</i>支付方式：</h3>
				<div class="info">
					<input value="1" name="payway" type="radio" class="inpt-radio" <c:if test="${goods.deposit != '0'}">checked="checked"</c:if>/> <i class="radio-right">定金支付</i>
					<input value="2" name="payway" type="radio" class="inpt-radio" <c:if test="${goods.deposit == '0' || goods.deposit == '100'}">checked="checked"</c:if>/> <i class="radio-right">不支持定金</i>
				</div>
			</div>
			<div class="info-box" id="deposit-radio" <c:if test="${goods.deposit != '0' && goods.deposit != '100'}">style="display: block;"</c:if>>
				<h3><i>*</i>定金比例：</h3>
				<div class="info">
					<input type="number" class="inpt-short" id="deposit" value="${goods.deposit}"/><i style="color:#e10000">（只限100内整数,无需输入百分号。）</i>
				</div>
			</div>
			...
			<div class="info-box">
				<h3><i>*</i>商品库存：</h3>
				<div class="info">
					<input class="inpt-short" id="goodsStock" type="number" value="${goods.goodsStock}"/>
				</div>
			</div>
			<div class="info-box">
				<h3>库存警告数量：</h3>
				<div class="info">
					<input class="inpt-short" id="stockEmergency" type="number" value="${goods.stockEmergency}"/>
				</div>
			</div>
			<div class="info-box">
				<h3><i>*</i>虚拟销量：</h3>
				<div class="info">
					<input class="inpt-short" id="fakeSales" type="number" value="${goods.fakeSales}"/>
				</div>
			</div>
			<!--<div class="info-box">
				<h3>预售设置：</h3>
			...
			<div class="info-box">
				...
				<div class="info">
					<div class="inpt-box-bg">
						<h5>
							<label>市场售价(￥)</label>
							<input id="marketPrice" type="number" value="${goods.marketPrice}"/>
						</h5>
						<h5>
							<label>京东售价(￥)</label>
							<input id="jdPrice" type="number" value="${goods.jdPrice}"/>
						</h5>
						<h5>
							<label>淘宝售价(￥)</label>
							<input id="tbPrice" type="number" value="${goods.tbPrice}"/>
						</h5>
					</div>
				</div>
			</div>
			<div class="info-box">
				<h3>淘宝在售商家：</h3>
				<div class="info">
					<input class="inpt-short" id="tSellStoreNnum" type="number" value="${goods.sellStoreNum}"/>
				</div>
			</div>
			<!--大额采购信息-->
			...
			<div class="info-box">
				<h3><i>*</i>大额采购：</h3>
				<div class="info">
					<input name="largeBuy" value="1" type="radio" class="inpt-radio" <c:if test="${goods.largeBuy != '2'}">checked="checked"</c:if>/> <i class="radio-right">支持</i>
					<input name="largeBuy" value="2" type="radio" class="inpt-radio" <c:if test="${goods.largeBuy == '2'}">checked="checked"</c:if>/> <i class="radio-right">不支持</i>
				</div>
			</div>			
			<div class="info-box" id="largeBuy-radio" <c:if test="${goods.largeBuy == '2'}">style="display:none"</c:if> >
				<h3>采购信息：</h3>
				<div class="info">
					<div class="inpt-box-bg" style="margin-top:0;">
						<h5>
							<label>托盘规格</label>
							<input id="largeNorm" value="${goods.largeNorm}"/>
						</h5>
						<h5>
							<label>最小起订量</label>
							<input id="largeMinNum" value="${goods.largeMinNum}"/>
						</h5>
					</div>
				</div>
			</div>
			...
			<div class="info-box">
				<h3>上架时间：</h3>
				<div class="info">
					<input name="goodsState" value="2" <c:if test="${goods.goodsState == '2'}">checked="checked"</c:if> type="radio" class="inpt-radio"/> <i class="radio-right">立即上架</i>
					<input name="goodsState" value="1" <c:if test="${goods.goodsState == '1'}">checked="checked"</c:if> type="radio" checked="checked" class="inpt-radio"/> <i class="radio-right">放入商品库</i>
				</div>
			</div>
			<div class="info-box">
				<h3><i>*</i>邮费计算方式：</h3>
				<div class="info">
					<input name="postageStyle" value="1" <c:if test="${goods.postageStyle == '1'}">checked="checked"</c:if> type="radio" class="inpt-radio"/> <i class="radio-right">包邮</i>
					<input name="postageStyle" value="2" <c:if test="${goods.postageStyle == '2'}">checked="checked"</c:if> type="radio" class="inpt-radio"/> <i class="radio-right">不包邮</i>
				</div>
			</div>
			<!--<div class="info-box">
				<h3>关联商品ID：</h3>
				<div class="info">
					<input class="inpt-short" id="relate1GID" value="${goods.relate1GID}"/>
					<input class="inpt-short" id="relate2GID" style="margin-left: 20px;"  value="${goods.relate2GID}"/>
				</div>
			</div>-->
		</div>
		<!--保存提交-->
		<div id="save-foot">
			<div id="save-good">保存修改</div>
		</div>
		<script>
			//类目选择
			//一级类目选择
					...
					success:function(data){
						...
						if(data.result == 'success'){
							...
							for(var i = 0;i < pn.length;i++){
								pn_child += "<h5><label>"+pn[i]+"</label><input class='property-list' pn='"+pn[i]+"'/></h5>"
							}
...
//			                		}
			...
			$(function(){
				//数据集合
				goodsVo = {};
				goodsVo.goodsID = $('#entry-box').attr('goodsID');
				cate = true;
				goodsVo.category1 = $('#category1 option:selected').text();
				goodsVo.category1ID = $('#category1').val();
				goodsVo.category2 = $('#category2 option:selected').text();
				...
				//定义关键词数组
				good_keyword = [];
				for(var i = 0;i < $('.good-keyword').length;i++){
					good_keyword.push($('.good-keyword').eq(i).text())
				}
...
//					console.log(good_keyword);
				...
				$('#save-good').on('click',function(){
					...
					for(var i = 0;i < $('.property-list').length;i++){
						var pn = $('.property-list').eq(i).siblings('label').text();
...
//					return console.log(goodsVo)
					layer.confirm("确定保存修改？", function(index) {
						$.ajax({
							type:'post',
							url:'noShelvesGoods/updateGoods',
							data:goodsVo,
							dataType:'json',
							success:function(data){
								if(data.result == 'success'){
